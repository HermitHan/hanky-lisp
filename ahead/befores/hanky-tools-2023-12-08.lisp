...
(defun nth-int (n alist)
  (parse-integer (nth n alist)))
...
(defun print-n (&rest objs)
  (mapcar #'print objs))
'(docs "一一打印 objs 多个内容")
'(keys (print))
'(example ((print-n 1 2 3)))

(defun print-list (alist)
  (dolist (i alist) (print i)))
'(docs "对一个列表, 打印列表中每个元素")
'(keys (print list))
'(example ((print-list '(1 2 3))
           (print-list '(1 (2 (3 4)) 5))))

(defun format-nx (n x &key (stream t) (before nil) (after nil))
  (when before (format stream before))
  (dotimes (i n) (format stream x))
  (when after (format stream after)))
'(docs "向 stream 流中 format n 次 x")
'(keys (format IO))
'(example ((format-nx 6 "-")
           (format-nx 6 "+" :before "!")
           (format-nx 5 "=" :after ">")
           (format-nx 5 "=" :before "<" :after ">")))

(defun alist-show (a-alist &key (stream t) (split ": "))
  (mapcar #'(lambda (i) (format stream "~A~A~S~%"
                                (car i) split (cdr i)))
          a-alist))
'(docs "将一个 alist 以 key: value 的方式打印出来")
'(keys (alist list show))
'(example ((alist-show '((a 1) (b 2) (c 3)))))
...
'(example ((make-reload "~/.hanky-lisp/load-hanky-lisp.lisp" (re-tools cre-tools))))
...
'(docs "对 source 没 n 个进行一次分组, 形成列表的列表, 对 n = 0 时进行了判断, 防止无限递归")
'(keys (list))
